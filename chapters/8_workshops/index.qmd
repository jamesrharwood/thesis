...
The aim of step 7 is to identify possible behaviour change techniques by systematically considering items from a taxonomy of 93 techniques for each intervention function. Asking this of EQUATOR was impractical; it was too time consuming to do as a group and EQUATOR staff did not have time to become familiar with the taxonomy.

#TODO: Is this different to how Michie recommends? I think she says that for each IF consider BCTs. If I had done it that way around, I would *still* have needed an ideas generation step. It's OK to decide in favour of a BCT for a particular IF in abstract, but what does that mean in practice? Still have to make content decisions. 

#Consider replacing "It was too time consuming to do as a group..." above with something like:
* Michie's recommended step was impractical because:
    * Required understanding and familiarity with BCTs
    * Takes a lot of time (Could cite https://research-information.bris.ac.uk/ws/portalfiles/portal/189192935/s40814_019_0425_6.pdf or @carvalhoParticipatoryDesignBehaviour2020)

In the original BCW paper, Michie writes that "The process of designing behaviour change interventions usually involves first of all determining the broad approach that will be adopted and then working on the specifics of the intervention design. For example, when attempting to reduce excessive antibiotic prescribing one may decide that an educational intervention is the appropriate approach. Alternatively, one may seek to incentivise appropriate prescribing or in some way penalise inappropriate prescribing. Once one has done this, one would decide on the specific intervention components. ". But in our case this wasn't really true. We were already constrained (imanitation wise and practicality wise) but a system that already exists and what parts of that system we could control. Instead of picking a particular "broad approach", we found ourselves considering how we could refine the existing system to utilize multiple approaches. Hence we were immediately (and naturally) talking about intervention components.  Instead, I asked EQUATOR staff to brainstorm ideas more loosely for each intervention function without worrying about a taxonomy. This "ideas generation" step was also an opportunity to solicit feedback from the broader reporting guideline community, which I describe fully in chapter {{< var chapters.focus-groups >}}. My workshops with EQUATOR generated {{< var counts.EQUATOR_ideas >}} which grew to {{< var counts.sub_items_pre_jh >}} ideas after input from stakeholders.

I then worked "backwards" to identify what BCTs these components were using. I then performed the step "forwards", as initially intended by Michie et al., by taking IFs in turn, considering all BCTs and especially those not already used in any existing ideas. I added {{< var  counts.jh_ideas >}} extra ideas that occurred to me during this systematic step, leading to {{< var.counts.sub_items >}} in total.

#TODO: I might want to strip out "delivery options" from the ideas table as they cannot be labelled with a BCT.

#TODO: Decide whether to move the intervention component table to here, or whether to add a different table here. Perhaps just 3 cols. Intervention Function; Behaviour Change Technique; Idea? Could label where these ideas came from - workshops / focus groups / me. 
...
* Homogenous group. Would have benefited from other stakeholders & authors but I try to make up for this. 
...
* It was difficult to think blue sky. Our thinking was clouded by what already exists. 
