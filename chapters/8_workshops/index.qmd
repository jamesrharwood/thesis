...
In the original BCW paper, Michie writes that "The process of designing behaviour change interventions usually involves first of all determining the broad approach that will be adopted and then working on the specifics of the intervention design. For example, when attempting to reduce excessive antibiotic prescribing one may decide that an educational intervention is the appropriate approach. Alternatively, one may seek to incentivise appropriate prescribing or in some way penalise inappropriate prescribing. Once one has done this, one would decide on the specific intervention components. ". But in our case, picking a "broad approach" didn't feel helpful. Given that a system already exists for disseminating reporting guidance, and given that EQUATOR can only control a part of that system, we found ourselves considering how we could use multiple approaches to refine the existing system. Hence we were immediately (and naturally) talking about intervention components. 
