...
The tenth question required exploring whether website users were able to access the _right_ resource. Although Google Analytics could tell me _what_ pages visitors access, it could not tell me what they _needed_, or _why_. To explore this, I decided to use PopupSmart @PopupBuilderThat to build an exit survey (an online questionnaire that pops up when a user appears to be leaving the website). I did not want to annoy users or block them from using the website, and I wanted to keep the survey short in the hope of maximizing repose rate, and so I decided to limit the survey to a single question. I decided to use an open ended question in the hope of receiving richer responses. 
